...
if (!class_exists('lib\Crypt\RSA') && !class_exists('Crypt_RSA')) {
...
{
	function OpenIDConnectClientException($msg){
		echo $msg;
	}
}
...
class OpenIDConnectClient
{
	...
	if(class_exists('Crypt_RSA')) {
		...
		$rsa = new lib\Crypt\RSA();
		$rsa->setHash($hashtype);
        	$rsa->loadKey($public_key_xml, CRYPT_RSA_PUBLIC_FORMAT_XML);
        	$rsa->signatureMode = CYRPT_RSA_SIGNATURE_PKCS1;
	}
    ...
    public function canVerifySignatures() {
      return class_exists('lib\Crypt\RSA') || class_exists('Crypt_RSA');
    }
